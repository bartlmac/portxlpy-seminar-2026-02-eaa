...
Your working directory (under `root`) is `/Bartek`.  
The Excel file is located in the `input` directory.  
All outputs generated by the LLM and by the scripts must be written to the `output` directory.  
...
- **Response format:** Each task delivers **exactly one** executable code block.  
...
1. **Input**: `Tariff_Calculator.xlsm`  
2. **Output**  
...
3. **Procedure**  
   ...
   - Ignore empty cells.  
   - Save CSVs in the same directory as the Excel file.  
   - A robust solution for ranges with invalid references is required.  
...
```  
5. **Delivery format**: Full, executable code block.
...
1. **Input**: `Tariff_Calculator.xlsm`  
2. **Output**: One `Mod_*.txt` file per VBA module  
3. **Procedure**  
...
## TASK 3 – Implement Code to Extract Data from Excel

1. **Input**: CSV files `excelcell.csv`, `excelrange.csv` (uploaded to LLM)  
2. **Output**  
...
1. **Input**: Existing CSV files (`var.csv`, `tariff.csv`, `tariff.py`, `limits.csv`, `tables.csv`) (uploaded to LLM)  
2. **Output**: Folder `tests/` with pytest fixtures  
3. **Procedure**  
...
1. **Input**: All `Mod_*.txt` from TASK 2 (uploaded to LLM)  
...
3. **Procedure**  
   ...
   - Available data sources: `excelcell.csv`, `excelrange.csv`, `var.csv`, `tariff.csv`, `limits.csv`, `tables.csv`.
...
## TASK 6A – Implement Code for NormGrossAnnualPrem()  (Premium calculation 1 of 4)

1. **Input**
   • `basfunct.py` (uploaded to LLM)
